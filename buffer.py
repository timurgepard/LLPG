...
class DDPG():
    ...
    def eps_step(self):
        ...
        if self.n_step<self.n_steps:
            self.x += 100*self.act_learning_rate
        self.tr += 1
...
if option == 1:
    ...
    critic_learning_rate = 0.001
    n_steps = 256
    x_step = 100*actor_learning_rate
elif option == 4:
    std_coef = 0.25
...
ddpg = DDPG(     env_name=env, # Gym environment with continous action space
                 ...
                 buffer=None,
                 n_steps = n_steps,
                 std_coef = std_coef, #standard deviation multiplier
                 x_step = x_step, #used in Eps = 1-sigmod(x)
