...
class _actor_network():
    ...
    def model(self):
        ...
        x = Dense(50, activation=atanh, kernel_initializer=RU(-1/np.sqrt(100),1/np.sqrt(100)))(x)
...
class _critic_network():
    ...
    def model(self):
        ...
        state_i = Dense(75, activation=atanh, kernel_initializer=RU(-1/np.sqrt(self.state_dim),1/np.sqrt(self.state_dim)))(state)
        #x = BatchNormalization()(state_i)
        action = Input(shape=(self.action_dim,), name='action_input')
        x = concatenate([state_i, action])
        x = Dense(50, activation=atanh, kernel_initializer=RU(-1/np.sqrt(101),1/np.sqrt(101)))(x)
        #x = BatchNormalization()(x)
